...
Wire AI tools into a running `<DocxEditor>` so `add_comment`, `suggest_change`, `insert_text`, `replace_text`, `set_content_control`, `set_content_control_value`, `insert_table`, `insert_image`, `find_text` etc. show up live in the user's editor.
...
Built-in agent tools (`read_document`, `find_text`, `read_content_controls`, `add_comment`, `suggest_change`, `insert_text`, `replace_text`, `set_content_control`, `set_content_control_value`, `insert_table`, `insert_image`, `read_comments`, `read_changes`, `reply_comment`, `resolve_comment`, `read_selection`, `read_page`, `read_pages`, `apply_formatting`, `set_paragraph_style`, `scroll`) are exposed automatically via MCP `tools/list` and `tools/call`. MCP spec version: `2025-06-18`.
